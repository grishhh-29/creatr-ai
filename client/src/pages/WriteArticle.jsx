...
const WriteArticle = () => {
	...
	const [remainingArticleCredits, setRemainingArticleCredits] = useState(null);

	const { getToken } = useAuth();

	// Preload user credits on component mount
	useEffect(() => {
		const fetchCredits = async () => {
			try {
				...
				if (data.success && data.credits?.article !== undefined) {
					setRemainingArticleCredits(data.credits.article);
				}
			} catch (error) {
	...
	const onSubmitHandler = async (e) => {
		...
		try {
			...
			if (data.success) {
				...
				// Update remaining article credits after generation
				if (data.remainingCredits !== undefined) {
					setRemainingArticleCredits(data.remainingCredits);
				}
			} else {
	...
	return (
		<div className="h-full overflow-y-scroll p-6 flex flex-col gap-4 text-slate-700">
			...
			{remainingArticleCredits === null ? null : (
			...
			{/* Second row: Article Config + Generated Article side by side */}
			<div className="flex flex-wrap gap-4">
				{/* Left col: Article Configuration */}
